...
const projectFiles = [
  ...
  ...fs
    ...
    .map((d) => `packages/${d.name}/project.json`)
    .filter((p) => fs.existsSync(p)),
];
const projects = new Map(
  projectFiles.map((file) => {
    const json = JSON.parse(fs.readFileSync(file, "utf8"));
    return [json.name, { ...json, root: path.dirname(file) }];
  })
);
const done = new Set();
...
function runTarget(name, target) {
  ...
  if (target === "build" || target === "typecheck" || target === "test")
    for (const dep of project.implicitDependencies ?? []) runTarget(dep, "build");
...
try {
  if (args[0] === "build" || args[0] === "dev") {
    ...
    const selected = args.includes("--all")
      ? [...projects.keys()].filter((name) => name !== "nx")
      : (parseOption("--projects=") ?? "").split(",").filter(Boolean);
    if (target === "dev") {
