...
import {
  ...
  linkedinJobImportInputSchema,
  LinkedinJobImportService,
  type NormalizedLinkedinJob,
} from "./linkedin-job-import-service.js";
...
test("linkedin import validation extracts currentJobId when missing", () => {
  const parsed = linkedinJobImportInputSchema.parse({
    sourceUrl: "https://www.linkedin.com/jobs/search/?currentJobId=123",
    rawText: "Useful job content",
  });
...
test("linkedin import validation rejects requests without useful content", () => {
  assert.throws(
    () => linkedinJobImportInputSchema.parse({ sourceUrl: "https://www.linkedin.com/jobs/view/123/" }),
    /useful job field/
  );
});
...
test("valid LinkedIn import maps LLM output into createOpportunity input", async () => {
  ...
  const service = new LinkedinJobImportService({
    ...
    findExistingByCompanyRole: async () => null,
...
test("duplicate LinkedIn import returns existing opportunity without normalizing or creating", async () => {
  ...
  const result = await service.importFromLinkedin(
    { sourceUrl: "https://www.linkedin.com/jobs/view/4428934873/", rawText: "Useful content" },
    "user@example.com"
  );
...
test("missing optional normalized fields do not break import", async () => {
  ...
  const result = await service.importFromLinkedin(
    { sourceUrl: "https://www.linkedin.com/jobs/view/1/", title: "Engineer" },
    "user@example.com"
  );
  assert.equal(result.created, true);
...
test("OpenAiLinkedinJobNormalizer keeps extractor metadata even if model returns different source metadata", async () => {
  const { OpenAiLinkedinJobNormalizer } = await import("./linkedin-job-import-service.js");
  const normalizer = new OpenAiLinkedinJobNormalizer({
  ...
  const result = await normalizer.normalize(
    linkedinJobImportInputSchema.parse({
      sourceUrl: "https://www.linkedin.com/jobs/view/4428934873/",
      rawText: "Useful content",
      extractedAt: "2026-06-28T12:00:00.000Z",
    })
  );

  assert.equal(result.metadata.sourceUrl, "https://www.linkedin.com/jobs/view/4428934873/");
  assert.equal(result.metadata.linkedinJobId, "4428934873");
  assert.equal(result.metadata.extractedAt, "2026-06-28T12:00:00.000Z");
});

test("LinkedIn import returns duplicate when normalized company and role match an existing manual opportunity", async () => {
  let createCalled = false;
  const service = new LinkedinJobImportService({
    normalizer: { normalize: async () => normalized },
    findDuplicate: async () => null,
    findExistingByCompanyRole: async (input, ownerEmail) => {
      assert.deepEqual(input, { companyName: "Example Company", roleTitle: "Senior Full Stack Engineer" });
      assert.equal(ownerEmail, "user@example.com");
      return { id: "manual_opp", companyName: input.companyName, roleTitle: input.roleTitle };
    },
    createOpportunity: async () => {
      createCalled = true;
  ...
  assert.equal(result.created, false);
  assert.equal(result.duplicate, true);
  assert.equal(result.opportunityId, "manual_opp");
  assert.match(result.warnings.join("\n"), /same company and role title/);
});
