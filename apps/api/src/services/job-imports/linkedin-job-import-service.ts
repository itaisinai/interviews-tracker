...
import { type AiParserService, getAiParserService } from "../ai/ai-parser-service.js";
...
export const linkedinJobImportInputSchema = z
  .object({
...
const linkedinJobImportJsonSchema = {
  ...
  properties: {
    company: {
      type: "object",
      additionalProperties: false,
      required: ["name", "linkedinUrl", "website", "description", "industry"],
      properties: {
        name: { type: "string" },
    ...
    opportunity: {
      ...
      properties: {
        ...
        workplaceType: { type: ["string", "null"], enum: ["remote", "hybrid", "onsite", null] },
        ...
        originalJobDescription: { type: ["string", "null"] },
      },
    },
    metadata: {
      type: "object",
...
export class OpenAiLinkedinJobNormalizer implements LinkedinJobNormalizer {
  constructor(
    private readonly aiParserService: Pick<AiParserService, "createStructuredOutput"> = getAiParserService()
  ) {}
  ...
  async normalize(input: LinkedinJobImportInput) {
    ...
    throw new Error(
      lastError instanceof Error && lastError.message.startsWith("OpenAI LinkedIn import failed")
        ? lastError.message
...
type ExistingOpportunityResult = {
  id: string;
...
export type LinkedinJobImportDependencies = {
  ...
  findExistingByCompanyRole?: (
    input: { companyName: string; roleTitle: string },
    ownerEmail: string
  ) => Promise<ExistingOpportunityResult | null>;
...
export class LinkedinJobImportService {
  ...
  private readonly findExistingByCompanyRoleRecord: NonNullable<
    LinkedinJobImportDependencies["findExistingByCompanyRole"]
  >;
  ...
  async importFromLinkedin(rawInput: unknown, ownerEmail: string) {
    ...
    if (existingByCompanyRole)
      return this.toResult(existingByCompanyRole, false, true, input, [
        ...normalized.warnings,
    ...
    const notes = [
      ...
      normalized.opportunity.responsibilities.length
        ? `Responsibilities:\n- ${normalized.opportunity.responsibilities.join("\n- ")}`
        : null,
      normalized.opportunity.requirements.length
        ? `Requirements:\n- ${normalized.opportunity.requirements.join("\n- ")}`
        : null,
      normalized.opportunity.niceToHave.length
        ? `Nice to have:\n- ${normalized.opportunity.niceToHave.join("\n- ")}`
        : null,
      normalized.opportunity.originalJobDescription
  ...
  private toResult(
    opportunity: ExistingOpportunityResult,
    created: boolean,
    duplicate: boolean,
    input: LinkedinJobImportInput,
    warnings: string[]
  ) {
    return {
      opportunity,
