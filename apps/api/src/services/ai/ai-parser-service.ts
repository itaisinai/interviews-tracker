...
export class OpenAiParserService implements AiParserService {
  ...
  async classifyGmailEmails(input: {
    ...
    const outputText = await this.createStructuredOutput({
      ...
      systemPrompt: [
        ...
        "",
        "STRICT FILTERING RULES - Mark as UNRELATED and isRelevant=false:",
        "1. Automated notifications:",
        "   - Google Calendar notifications/reminders (from calendar-notification@google.com, calendar@google.com)",
        "   - System error notifications (PrismaClientValidationError, alerts, exceptions)",
        "   - Automated system emails (from noreply@, no-reply@, donotreply@, notifications@)",
        "   - Vendor platform notifications (e.g., *-notifications.com domains)",
        "",
        "2. Pure reminder emails with no content:",
        "   - Subject starting with 'Notification:', 'Reminder:', 'Alert:'",
        "   - Emails that just remind about a calendar event without additional context",
        "   - Automated meeting reminders from third-party platforms",
        "",
        "3. Meeting invitations that are NOT job-related:",
        "   - Personal meetings, team meetings with current employer",
        "   - Events that don't involve interviews or recruitment",
        "",
        ...
        isGeneralOpportunitySearch
          ? [
              ...
              "Be EXTRA strict. Mark as RELEVANT only if:",
              "- Direct recruiter outreach about job opportunities (from a real person)",
              "- Job interview invitations WITH substantive content (not just automated reminders)",
              "- Offer letters or employment discussions",
              "- Actual job application follow-ups from companies",
              "",
              "KEY: If the email body/snippet contains NO meaningful content beyond 'you have a meeting',",
              "it's just a notification → mark as UNRELATED.",
            ].join("\n")
          : [
              `Target company: ${input.companyName}`,
              input.roleTitle ? `Target role: ${input.roleTitle}` : null,
              "",
              "For company-specific searches:",
              "- Keep recruiter messages, interview invitations, follow-ups from the target company",
              "- Still filter out pure notifications/reminders even if from the target company",
              "- If the email is just an automated reminder with meeting details but no content, mark as UNRELATED",
            ]
              .filter(Boolean)
              .join("\n"),
  ...
  async parseStructuredGmailEmailToInteraction(input: {
    ...
    const outputText = await this.createStructuredOutput({
      ...
      systemPrompt: [
        ...
        "- Extract personName and personRole from interviewer mentions (e.g., 'Interview with [Name]', '[Name] (Title)')",
        "- Always extract meetingLink from video conference URLs (meet.google.com, zoom.us, etc.)",
        "- For notes: preserve ALL actionable prep details (interviewer, format, requirements, language options, special instructions)",
        "- Include technical requirements (camera on, screen sharing, tools needed)",
        "- Include interview format/style (LeetCode-style, behavioral, case study)",
        "- Include phone dial-in numbers with PINs if provided",
        "- Do NOT include redundant metadata (subject, from, date) in notes",
        "",
        "Fill agenda, notes, outcome, followUp, and personRole comprehensively when explicit in the structured email.",
    ...
    return interactionDraftSchema.parse({
      ...
      // Use AI-generated notes directly - derived notes are already included in the AI prompt as context
      // and the AI has been instructed to extract comprehensive details, so no need to append raw metadata
  ...
  async parseMultipleEmailsToInteraction(input: {
    ...
    const systemPrompt = [
      ...
      "",
      "Person extraction:",
      "- Extract personName and personRole from: email signatures, calendar organizer, or explicit mentions like 'You'll be meeting with [Name] ([Role])'",
      "- Look for patterns like: 'Interview with [Name]', '[Name] (Banking Team Lead)', 'Interviewer: [Name]'",
      "- Check LinkedIn profile links or role mentions in the body",
      "",
      "Meeting link extraction:",
      "- Always extract meetingLink from: Google Meet URLs (meet.google.com/*), Zoom links (zoom.us/* or us0*.zoom.us/*), Microsoft Teams, or other video conference URLs",
      "- Prefer explicit 'Join with Google Meet' or 'Join via Zoom' links over plain URLs in text",
      "",
      "Notes extraction (COMPREHENSIVE, not just summary):",
      "- Extract ALL actionable interview preparation details including:",
      "  * Interviewer name/role/title (if not already in personName/personRole)",
      "  * Meeting format (video, phone, in-person) and location details",
      "  * Technical requirements: camera on/off, screen sharing, specific tools/software",
      "  * What to prepare or bring (laptop, whiteboard, portfolio)",
      "  * Interview format/style (behavioral, technical, LeetCode-style, case study, etc.)",
      "  * Coding language options or preferences (e.g., 'Python preferred, JS/TS/Node available')",
      "  * Special instructions (AI recording notice, opt-out options, parking info)",
      "  * Dial-in phone numbers with PINs if provided as backup",
      "  * Dress code, office location, who to ask for, building access codes",
      "- Do NOT include redundant metadata already captured in other fields (subject, from, date)",
      "- Keep comprehensive but organized - use bullet points or short paragraphs for readability",
      "- Preserve specific details like: 'Ido Raz (Banking Team Lead)', 'LeetCode-style practical problem', 'Camera must be on'",
      "",
      "Type/Stage:",
