...
async function getAccessTokenForEmail(auth0Email: string, settings = requireSettings()) {
  ...
  try {
    ...
    if (connection.needsReconnect || connection.lastError) {
...
export async function getGmailStatus(auth0Email: string): Promise<GmailStatus> {
  ...
  return {
    configured: true,
    connected: Boolean(connection && !connection.needsReconnect),
...
export async function completeGmailOAuth(code: string, state: string) {
  ...
  try {
    ...
    const refreshToken = tokens.refresh_token ?? (
      existingConnection
        ? decryptText(existingConnection.refreshTokenEncrypted, settings.encryptionSecret)
        : null
    );
