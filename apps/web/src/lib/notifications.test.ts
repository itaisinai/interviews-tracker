...
import { activeNotifications, buildUnlinkedInteractionNotifications, syncNotifications, unreadNotificationsCount } from "./notifications.js";

const now = new Date("2026-06-15T12:00:00.000Z");
const later = new Date("2026-06-15T12:05:00.000Z");
const base = [
  { id: "i1", jobOpportunityId: "reevol", gmailMessageId: null, jobOpportunity: { companyName: "Reevol" } },
  { id: "i2", jobOpportunityId: "reevol", gmailMessageId: undefined, jobOpportunity: { companyName: "Reevol" } },
  { id: "i3", jobOpportunityId: "alta", gmailMessageId: "gmail-1", jobOpportunity: { companyName: "Alta" } },
  { id: "i4", jobOpportunityId: "token", gmailMessageId: null, jobOpportunity: { companyName: "Token Security" } },
] as const;
...
test("detects unlinked interactions and groups notifications by opportunity", () => {
  ...
  assert.equal(notifications.find((item) => item.opportunityId === "token")?.title, "Token Security has 1 interaction not linked to emails");
});
...
test("resolves notification when all interactions are linked", () => {
  ...
  assert.equal(synced.every((item) => item.status === "resolved"), true);
});
