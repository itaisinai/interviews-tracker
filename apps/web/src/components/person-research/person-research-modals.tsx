...
export function ConfirmResearchModal({
  isOpen,
  onClose,
  person,
  onStartResearch,
}: ConfirmResearchModalProps) {
  ...
  const validateAndStart = () => {
    ...
    if (
      linkedinUrl.trim() &&
      !linkedinUrl.match(/^https?:\/\/(www\.)?linkedin\.com\//i)
    ) {
      ...
      <div className="space-y-5">
        ...
        <div className="rounded-lg bg-surface-container-low p-4">
          <p className="text-body-md text-on-surface-variant">
            We'll search public sources, including LinkedIn, to find information
            about this person.
          </p>
        ...
        <div className="flex items-center gap-3">
          ...
          <label htmlFor="saveForLater" className="flex-1 cursor-pointer">
            <span className="block font-label-md text-label-md">
...
export function ReviewResearchModal({
  ...
  const experienceData: CompanyExperience[] = (research.experience || []).map(
    (exp: any) => {
          ...
          <div>
            ...
            {experienceData.length > 2 ? (
              ...
              >
                ...
                {showAllExperience
                  ? "Show less"
                  : `Show ${experienceData.length - 2} more`}
              </button>
            ) : null}
          </div>
        ) : null}
        ...
        {research.education && research.education.length > 0 ? (
          <div>
            ...
            {research.education.length > 2 ? (
              ...
              >
                <span
                  className={`transition-transform duration-300 ${showAllEducation ? "rotate-180" : "rotate-0"}`}
                >
                  <ChevronDown className="h-4 w-4" />
                </span>
                {showAllEducation
                  ? "Show less"
                  : `Show ${research.education.length - 2} more`}
              </button>
            ) : null}
          </div>
        ) : null}
...
export function ResearchErrorModal({
  isOpen,
  onClose,
  onRetry,
  message,
