    ...
    <BreadcrumbsContext.Provider value={{ breadcrumbs, setBreadcrumbs }}>
      <div className="min-h-screen bg-background text-on-background">
        ...
        {breadcrumbs.length > 0 && (
          <div
            className="fixed inset-x-0 z-40 border-b border-outline-variant bg-surface-container-lowest/95 backdrop-blur-sm"
            style={{
              top: `calc(4rem + var(--dev-banner-height, 0))`,
              left: isDesktop ? `${sidebarWidth}px` : "0",
              transition: `left ${transitionDuration}s ease-out`,
            }}
          >
            <div className="mx-auto max-w-[1280px] px-4 py-2 md:px-6">
        ...
        <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-outline-variant bg-background/95 backdrop-blur-md pb-[env(safe-area-inset-bottom)] md:hidden">
          <div className="grid grid-cols-5">
            {nav
              .filter((item) => ["/", "/opportunities", "/interactions", "/search", "/settings"].includes(item.to))
              .map((item) => (
                <NavLink
                  key={item.to}
                  to={item.to}
        ...
        <main
          ...
          style={{
            paddingTop:
              breadcrumbs.length > 0
                ? "calc(7rem + var(--dev-banner-height, 0))"
                : "calc(4rem + var(--dev-banner-height, 0))",
            marginLeft: isDesktop ? `${sidebarWidth}px` : "0",
            transition: `margin-left ${transitionDuration}s ease-out, padding-top ${transitionDuration}s ease-out`,
          }}
        >
