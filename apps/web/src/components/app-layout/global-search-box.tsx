    ...
    <div ref={wrapperRef} className="relative w-full max-w-md">
      ...
      <input
        ...
        onKeyDown={(event) => {
          ...
          if (event.key === "Enter" && query.trim()) goToSearch();
        }}
      />
      {query ? (
        <button
      ...
      {shouldShowDropdown ? (
        <div className="absolute left-0 top-[calc(100%+8px)] z-[9999] w-full overflow-hidden rounded-2xl border border-outline-variant bg-surface-container-lowest shadow-xl">
          <div className="max-h-[420px] overflow-auto py-3">
            {isLoading ? (
              <p className="px-5 py-8 text-center text-body-md text-on-surface-variant">Searching...</p>
              ...
              <div className="px-5 py-12 text-center">
                <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-primary-container/40 text-primary">
                  <MaterialIcon name="search" />
                </div>
                <p className="font-title-sm text-title-sm font-bold">No results found</p>
                <p className="mt-1 text-body-md text-on-surface-variant">Try searching for something else</p>
              </div>
            )}
          </div>
          <button
