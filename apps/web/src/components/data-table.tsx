...
export function DataTable<TData>({ data, columns, emptyState, className, tableClassName, getRowProps }: DataTableProps<TData>) {
...
function TableBody<TData>({ table, emptyState, getRowProps }: { table: Table<TData>; emptyState?: ReactNode; getRowProps?: (row: TData) => HTMLAttributes<HTMLTableRowElement> }) {
  ...
  return (
    <tbody className="divide-y divide-outline-variant">
      {rows.map((row) => (
        <tr key={row.id} className="bg-surface-container-lowest transition-colors hover:bg-surface-container-low/80" {...(getRowProps?.(row.original) ?? {})}>
          {row.getVisibleCells().map((cell) => (
            <td key={cell.id} className="overflow-hidden border-r border-outline-variant/25 px-6 py-5 align-middle last:border-r-0" style={{ width: cell.column.getSize() }}>
