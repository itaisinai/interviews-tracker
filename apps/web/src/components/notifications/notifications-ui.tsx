...
export function NotificationWarningIcon() {
  return (
    <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-amber-500 text-white">
      <MaterialIcon name="priority_high" className="text-[18px]" />
    </span>
  );
}
...
export function NotificationsDropdown({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div
      className="absolute right-0 top-12 z-50 w-[min(92vw,500px)] overflow-hidden rounded-lg border border-outline-variant bg-background shadow-xl"
      role="dialog"
          ...
          latest.map((item) => (
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                <span className="block font-label-md text-label-md font-bold text-on-background">{item.title}</span>
                <span className="mt-1 block font-body-sm text-body-sm text-on-surface-variant">{item.message}</span>
              </span>
...
export function NotificationRow({ notification, onClick }: { notification: AppNotification; onClick: () => void }) {
  return (
    ...
    >
      <NotificationWarningIcon />
      <span className="min-w-0 flex-1">
        <span className="block font-label-md text-label-md font-bold text-on-background">{notification.title}</span>
        <span className="mt-1 block font-body-sm text-body-sm text-on-surface-variant">{notification.message}</span>
      </span>
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {unreadCount > 0 ? (
          <span
            className="absolute -right-0.5 -top-0.5 flex h-5 min-w-5 items-center justify-center rounded-full bg-primary px-1 font-label-sm text-[11px] font-bold text-on-primary"
